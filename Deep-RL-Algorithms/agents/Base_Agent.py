...
class Base_Agent(object):
    ...
    def get_state_size(self):
        ...
        if isinstance(random_state, dict):
            ...
            return random_state.shape
            # try:
            #     print("Random state", random_state.shape)
            #     return int(random_state.size)
            # except TypeError:
    ...
    def run_n_episodes(self, num_episodes=None, show_whether_achieved_goal=True, save_and_print_results=True):
        ...
        while self.episode_number < num_episodes:
            ...
            self.step()
            # print("Here2")
            if save_and_print_results and self.time_for_critic_and_actor_to_learn(): self.save_and_print_result()
            # print("Here3")
            if self.config.save_model and self.global_step_number % self.config.save_every_n_steps == 0 and self.time_for_critic_and_actor_to_learn():
    ...
    def conduct_action(self, action):
        ...
        if self.config.render:
            ...
            time.sleep(1e-3)
        self.next_state, self.reward, d, t, _ = self.environment.step(action)
        self.done = d or t
    ...
    def print_rolling_result(self):
        ...
        text = f""""\n Episode {len(self.game_full_episode_scores):0}, Total Steps {self.global_step_number:0}, Score: {self.rolling_results[-1]:.3f}, Max score seen: {self.max_rolling_score_seen:.3f}, Rolling score: {self.game_full_episode_scores[-1]:.3f}, Max rolling score seen: {self.max_episode_score_seen:.3f}, Time Taken (sec): {(time.time() - self.start):.3f}, Learning? {self.time_for_critic_and_actor_to_learn()}\n"""
    ...
    def take_optimisation_step(self, optimizer, network, loss, clipping_norm=None, retain_graph=False, name = ""):
    ...
    def create_CNN(self, input_dim, output_dim, key_to_use=None, override_seed=None, hyperparameters=None):
        ...
        return CNN(input_dim=input_dim, layers_info=hyperparameters["linear_hidden_units"] + [output_dim],
                  output_activation=hyperparameters["final_layer_activation"],
                  batch_norm=hyperparameters["batch_norm"], dropout=hyperparameters["dropout"],
                  hidden_activations=hyperparameters["hidden_activations"], initialiser=hyperparameters["initialiser"],
                #   columns_of_data_to_be_embedded=hyperparameters["columns_of_data_to_be_embedded"],
