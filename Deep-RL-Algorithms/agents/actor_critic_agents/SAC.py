...
LOG_SIG_MAX = 2
LOG_SIG_MIN = -20
TRAINING_EPISODES_PER_EVAL_EPISODE = 25
EPSILON = 1e-6
...
class SAC(Base_Agent):
    ...
    def step(self):
        ...
        while not self.done:
            # print(self.episode_step_number_val, ", ", self.environment._max_episode_steps, ", ", self.done)
            ...
            if self.time_for_critic_and_actor_to_learn():
                for _ in range(self.hyperparameters["learning_updates_per_learning_session"]):
                    self.learn()
            mask = False if self.episode_step_number_val >= self.max_episode_steps else self.done
    ...
    def update_critic_parameters(self, critic_loss_1, critic_loss_2):
        ...
        self.take_optimisation_step(self.critic_optimizer, self.critic_local, critic_loss_1,
                                    self.hyperparameters["Critic"]["gradient_clipping_norm"], name="Critic1")
        self.take_optimisation_step(self.critic_optimizer_2, self.critic_local_2, critic_loss_2,
                                    self.hyperparameters["Critic"]["gradient_clipping_norm"], name="Critic2")
    ...
    def update_actor_parameters(self, actor_loss, alpha_loss):
        ...
        self.take_optimisation_step(self.actor_optimizer, self.actor_local, actor_loss,
                                    self.hyperparameters["Actor"]["gradient_clipping_norm"], name="Actor")
        if alpha_loss is not None:
            self.take_optimisation_step(self.alpha_optim, None, alpha_loss, None, name="Alpha")
            self.alpha = self.log_alpha.exp()
