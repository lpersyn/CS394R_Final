...
class FrameStack(gym.Wrapper):
    ...
    def _get_ob(self):
        ...
        return LazyFrames(list(self.frames), self.observation_space.shape)
...
class LazyFrames(object):
    ...
    def _force(self):
        if self._out is None:
            # print("self._frames: ", [frame.shape for frame in self._frames])
...
class SwapChannelOrder(gym.ObservationWrapper):
    def __init__(self, env):
        gym.ObservationWrapper.__init__(self, env)
        self.observation_space = gym.spaces.Box(low=0, high=1, shape=(env.observation_space.shape[-1], env.observation_space.shape[0], env.observation_space.shape[1]), dtype=np.float32)
