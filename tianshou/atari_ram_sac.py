...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument("--task", type=str, default="SpaceInvaders-ramNoFrameskip-v4")
    ...
    class Net(NetBase):
        def __init__(self, state_shape, action_shape, hidden_size=64):
            ...
            self.net = nn.Sequential(
                nn.Linear(state_shape[0], action_shape),
                nn.ReLU(inplace=True)
            )
            self.output_dim = action_shape
            self.state_shape = state_shape
        def forward(self, x, state=None, info=None):
            x = x.reshape(-1, self.state_shape[0])
            return self.net(torch.as_tensor(x, dtype=torch.float32)), state
